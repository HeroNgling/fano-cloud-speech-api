...
1. **Submit the file:** You send a `POST` request to the `/api/v1/transcript` endpoint with your audio file (as a URI or base64-encoded string) and configuration.
...
def submit_job(audio_uri):
    ...
    """
    submit_url = f"{BASE_URL}/api/v1/transcript"
...
def get_results(operation_id):
    ...
    """
    poll_url = f"{BASE_URL}/api/v1/transcript/{operation_id}"
...
const BASE_URL = "https://studio.fano.ai";
...
async function submitJob(audioUri) {
    const submitUrl = `${BASE_URL}/api/v1/transcript`;
...
async function getResults(operationId) {
    const pollUrl = `${BASE_URL}/api/v1/transcript/${operationId}`;
